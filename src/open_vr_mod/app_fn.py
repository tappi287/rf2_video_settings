...
import open_vr_mod
import open_vr_mod.mod
import open_vr_mod.util.utils
from open_vr_mod.app_settings import AppSettings
from open_vr_mod.mod import get_available_mods
from open_vr_mod.util.manifest_worker import run_update_steam_apps
from open_vr_mod.util.custom_app import create_custom_app, scan_custom_library
from open_vr_mod.util.utils import get_name_id
...
def reduce_steam_apps_for_export(steam_apps) -> dict:
    ...
    for app_id, entry in steam_apps.items():
        ...
        if entry.get('openVr') or entry.get('vrpInstalled'):
            for mod in open_vr_mod.mod.get_available_mods(entry):
...
def load_steam_lib_fn():
    ...
    if open_vr_mod.globals.get_version() != AppSettings.previous_version:
...
def scan_app_lib_fn():
    ...
    try:
        ...
        open_vr_mod.steam.apps.read_steam_library()

        # -- Create a local copy of Steam Apps dict
        steam_apps = dict()
        steam_apps.update(open_vr_mod.steam.apps.steam_apps)

        # -- Remove Library Paths helper entry
        steam_apps.pop(open_vr_mod.steam.STEAM_LIBRARY_FOLDERS)
...
def add_custom_app_fn(app_dict: dict):
    ...
    for entry_id, entry in AppSettings.load_custom_dir_apps().items():
        if not entry_id.startswith(open_vr_mod.globals.USER_APP_PREFIX):
            continue
    ...
    if open_vr_mod.globals.USER_APP_PREFIX not in AppSettings.user_app_directories:
        AppSettings.user_app_directories[open_vr_mod.globals.USER_APP_PREFIX] = open_vr_mod.globals.get_settings_dir().as_posix()
        AppSettings.save()

    # -- Create User App entry
    app_id = f'{open_vr_mod.globals.USER_APP_PREFIX}_{get_name_id(path.stem)}'
    ...
    result = AppSettings.save_custom_dir_apps(open_vr_mod.globals.USER_APP_PREFIX, reduce_steam_apps_for_export(user_apps))
...
def add_custom_dir_fn(path: str):
    ...
    new_dir_id = f'{open_vr_mod.globals.CUSTOM_APP_PREFIX}{get_name_id(path.as_posix())}'
...
def set_mod_dir_fn(directory_str, mod_type: int):
    ...
    if not directory_str:
        ...
        open_vr_mod.mod.update_mod_data_dirs()
        result = True

    # -- Set
    if open_vr_mod.mod.check_mod_data_dir(Path(directory_str), mod_type):
